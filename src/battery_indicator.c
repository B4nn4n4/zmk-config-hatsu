...
static int set_default_indicator(void) {
    int err = set_color_all_leds(CONFIG_ZMK_BATTERY_INDICATOR_DEFAULT_COLOR, 100);
    if (err) {
...
static int zmk_battery_indicator_update(void) {
...
#if IS_ENABLED(CONFIG_ZMK_BATTERY_INDICATOR_BRIGHTNESS)
...
#if IS_ENABLED(CONFIG_ZMK_BATTERY_INDICATOR_BAR)
...
static int zmk_battery_indicator_init(void) { return set_default_indicator(); }
